...
import torch
from utils.utils import quat_from_euler_xyz as quat_from_euler_xyz_tensor
from utils.utils import quat_rotate_inverse, quat_mul
import torch

def tensor_clamp(x: torch.Tensor, min_v: torch.Tensor, max_v: torch.Tensor) -> torch.Tensor:
    """Clamp tensor x element-wise between min_v and max_v (broadcastable)."""
    return torch.max(torch.min(x, max_v), min_v)

def torch_rand_float_tensor(min_t: torch.Tensor, max_t: torch.Tensor) -> torch.Tensor:
    """Sample uniformly per element between min_t and max_t (broadcastable)."""
    return min_t + (max_t - min_t) * torch.rand_like(min_t)
